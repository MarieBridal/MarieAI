        ...
        <div className="glass-effect p-6 rounded-3xl border border-slate-700 shadow-2xl space-y-6 sticky top-6">
          ...
          <div className="grid grid-cols-3 gap-3">
            <motion.button
              ...
              className="py-5 border-2 border-dashed rounded-2xl flex flex-col items-center gap-2 bg-slate-900/40 border-slate-700 hover:border-blue-500 hover:bg-blue-500/5 transition-all group"
            >
              <Images className="w-5 h-5 text-slate-500 group-hover:text-blue-400 transition-colors" />
              <span className="text-[8px] text-slate-500 group-hover:text-blue-400 font-black uppercase tracking-widest transition-colors">THÊM HÌNH</span>
            </motion.button>
            <motion.button
              whileHover={{ scale: 1.02 }}
              whileTap={{ scale: 0.98 }}
              onClick={() => bgFileInputRef.current?.click()}
              className="py-5 border-2 border-dashed rounded-2xl flex flex-col items-center gap-2 bg-slate-900/40 border-slate-700 hover:border-emerald-500 hover:bg-emerald-500/5 transition-all group"
            >
              {bgImage ? <img src={bgImage} className="w-8 h-8 rounded-lg object-cover shadow-lg" /> : <Images className="w-5 h-5 text-slate-500 group-hover:text-emerald-400 transition-colors" />}
              <span className="text-[8px] text-slate-500 group-hover:text-emerald-400 font-black uppercase tracking-widest transition-colors truncate w-full px-2">THÊM NỀN</span>
            </motion.button>
            <motion.button
              whileHover={{ scale: 1.02 }}
              whileTap={{ scale: 0.98 }}
              onClick={() => refFileInputRef.current?.click()}
              className="py-5 border-2 border-dashed rounded-2xl flex flex-col items-center gap-2 bg-slate-900/40 border-slate-700 hover:border-purple-500 hover:bg-purple-500/5 transition-all group"
            >
              {refImage ? <img src={refImage} className="w-8 h-8 rounded-lg object-cover shadow-lg" /> : <Palette className="w-5 h-5 text-slate-500 group-hover:text-purple-400 transition-colors" />}
              <span className="text-[8px] text-slate-500 group-hover:text-purple-400 font-black uppercase tracking-widest transition-colors">STYLE MẪU</span>
            </motion.button>
          </div>
        ...
        <div ref={containerRef} onMouseDown={e => { if (e.button === 1 || isSpacePressed || activeItem?.selectedResultIndex !== -1) { setIsPanning(true); setPanStart({ x: e.clientX - offset.x, y: e.clientY - offset.y }) } }} onMouseMove={e => { setMousePos({ x: e.clientX, y: e.clientY }); if (isPanning) { setOffset({ x: e.clientX - panStart.x, y: e.clientY - panStart.y }) } }} onMouseUp={() => setIsPanning(false)} onMouseEnter={() => setIsHovering(true)} onMouseLeave={() => setIsHovering(false)} className={`flex-1 glass-effect rounded-[3rem] relative overflow-hidden flex flex-col items-center justify-center border border-slate-700 bg-slate-950/40 min-h-[500px] shadow-inner ${canvasActive && activeItem?.selectedResultIndex === -1 && !isSpacePressed ? 'cursor-none' : 'cursor-default'}`}>
          {currentImage ? (
            <div style={{ transform: `translate3d(${offset.x}px, ${offset.y}px, 0) scale(${zoom})`, transition: isPanning || isDrawing ? 'none' : 'transform 0.15s' }} className="relative inline-block rounded-2xl bg-black shadow-[0_50px_100px_rgba(0,0,0,0.8)]">
              ...
              <canvas ref={displayCanvasRef} onMouseDown={startDrawing} onMouseMove={draw} onMouseUp={stopDrawing} className={`absolute inset-0 z-10 w-full h-full rounded-2xl ${canvasActive && activeItem?.selectedResultIndex === -1 ? 'opacity-100 pointer-events-auto' : 'opacity-0 pointer-events-none'}`} />
              ...
              {activeItem?.detectedTexts && activeItem.detectedTexts.map((txt) => (
                <input
                  ...
                  className="absolute z-20 bg-transparent outline-none border border-dashed border-transparent hover:border-blue-500 focus:border-blue-500 focus:bg-black/40 transition-colors p-1"
                  style={{
                    left: `${txt.x * 100}%`,
                    top: `${txt.y * 100}%`,
                    color: txt.hexColor,
                    fontSize: `${txt.fontSize * 100}vh`, /* Simplified relative sizing */
              ...
              {canvasActive && (
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="flex items-center gap-2">
                  ...
                  <div className="flex items-center gap-3 bg-slate-900/80 backdrop-blur-md px-6 rounded-2xl border border-slate-700 shadow-xl h-14">
                    <span className="text-[9px] font-black text-slate-500">Cọ</span>
