        ...
        <div className="fixed inset-0 z-[100] bg-black/95 flex items-center justify-center p-4">
          <div className="w-full max-w-5xl bg-slate-900 rounded-[3rem] p-8 space-y-6 border border-slate-700 animate-slideUp">
            ...
            <div ref={modalEditorZoneRef} onMouseDown={e=>{if(e.button===1||isSpacePressed){setIsPanning(true);setPanStart({x:e.clientX-offset.x,y:e.clientY-offset.y})}}} onMouseMove={e=>{setMousePos({x:e.clientX,y:e.clientY});if(isPanning){setOffset({x:e.clientX-panStart.x,y:e.clientY-panStart.y})}}} onMouseUp={()=>setIsPanning(false)} onMouseEnter={()=>setIsHoveringModal(true)} onMouseLeave={()=>{setIsPanning(false);setIsHoveringModal(false)}} className={`relative border border-slate-700 rounded-2xl overflow-hidden bg-black flex items-center justify-center h-[60vh] ${isPanning||isSpacePressed ? 'cursor-grab' : 'cursor-none'}`}>
              <div style={{ transform: `translate3d(${offset.x}px, ${offset.y}px, 0) scale(${zoom})`, transition: isPanning||isDrawing ? 'none' : 'transform 0.1s' }}>
                <div className="relative inline-block">
                  ...
                  <canvas ref={modalDisplayCanvasRef} onMouseDown={e=>{if(!isSpacePressed){setIsDrawing(true);const rect=modalDisplayCanvasRef.current!.getBoundingClientRect();const x=(e.clientX-rect.left)/zoom;const y=(e.clientY-rect.top)/zoom;const dctx=modalDisplayCanvasRef.current!.getContext('2d')!;const mctx=modalCanvasRef.current!.getContext('2d')!;const sx=modalCanvasRef.current!.width/modalDisplayCanvasRef.current!.width;dctx.beginPath();dctx.moveTo(x,y);mctx.beginPath();mctx.moveTo(x*sx,y*sx)}}} onMouseMove={e=>{if(isDrawing){const rect=modalDisplayCanvasRef.current!.getBoundingClientRect();const x=(e.clientX-rect.left)/zoom;const y=(e.clientY-rect.top)/zoom;const dctx=modalDisplayCanvasRef.current!.getContext('2d')!;const mctx=modalCanvasRef.current!.getContext('2d')!;const sx=modalCanvasRef.current!.width/modalDisplayCanvasRef.current!.width;const bs=brushSize/zoom;dctx.lineWidth=bs;dctx.lineTo(x,y);dctx.stroke();mctx.lineWidth=bs*sx;mctx.lineTo(x*sx,y*sx);mctx.stroke()}}} onMouseUp={()=>setIsDrawing(false)} className={`absolute top-0 left-0 w-full h-full z-10 ${isSpacePressed ? 'pointer-events-none' : 'pointer-events-auto'}`} />
            ...
            <div className="flex flex-col md:flex-row justify-between items-center gap-6 pt-6 border-t border-slate-800">
              <div className="flex items-center gap-4 w-full md:w-auto"><span className="text-[10px] font-black text-slate-500 uppercase">Cọ: {brushSize}px</span><input type="range" min="5" max="250" value={brushSize} onChange={(e) => setBrushSize(parseInt(e.target.value))} className="w-full md:w-64 h-1.5 accent-red-500" /></div>
              <div className="flex gap-3 w-full md:w-auto">
                <button onClick={()=>{const dctx=modalDisplayCanvasRef.current! .getContext('2d')!;const mctx=modalCanvasRef.current!.getContext('2d')!;dctx.clearRect(0,0,9999,9999);mctx.clearRect(0,0,9999,9999)}} className="px-6 py-3 text-[10px] font-black text-red-500">Xóa Mask</button>
                <button onClick={()=>{setItems(prev=>prev.map(it=>it.id===editingItem.id?{...it,mask:modalCanvasRef.current!.toDataURL()}:it));setEditingItem(null)}} className="bg-blue-600 px-10 py-3 rounded-2xl text-[10px] font-black text-white">Lưu Mask</button>
              </div>
            </div>
          </div>
        </div>
